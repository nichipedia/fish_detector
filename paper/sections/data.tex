...
The dataset for this project is broken into test, validate, and train sections. Total contains 10,000 images that are stored in full HD 1920 by 1080 resolution. A stratified splitting method was used to maintain the proportion of samples of each class in the Validate, test, and train portions. The below table represents the sample count in the training dataset, but the percentage will give a basis on what each looks like in the other two portions relative to other classes. The Fish 1 class represents the largest portion of the data. It is twice as large as the second  
...
The table below represents the percentage of fish samples and no fish samples. No fish has a 14\% higher representation in the training data.  
...
Edits were made to the dataset to make development easier. Two new datasets were created. A dataset where fish and no-fish directories were copied to a new positive and negative dataset. Another was created where each individual class was copied to a folder containing its samples. This was done because the image loader used from PyTorch will automatically assign labels based on the folder names. Table \ref{tab:fish_classes} describes the classes that were created. A no fish class was also
